...
namespace Ergus.Backend.Application.Tests
{
    public class AdvertisementSkuPriceServiceTest : BaseTest
    {
        ...
        public async Task ShouldAddSuccessfully()
        {
            ...
            AssertHelper.AssertAddUpdate<AdvertisementSkuPrice>(actual);
        ...
        public async Task ShouldGetAllSuccessfully()
        {
            ...
            this._mockAdvertisementSkuPriceRepository.Setup(x => x.GetAll(0, 0, true)).ReturnsAsync(expected);

            var actual = await _service.GetAll(0, 0, true);

            this._mockAdvertisementSkuPriceRepository.Verify(x => x.GetAll(0, 0, true), Times.Exactly(1));
