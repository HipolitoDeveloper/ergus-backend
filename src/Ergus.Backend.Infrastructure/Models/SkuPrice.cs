...
{
    [Table("sku_preco")]
    public class SkuPrice : BaseModel, ISkuPrice, IGeneric
    {
        public SkuPrice() { }

        public SkuPrice(int id, string code, string externalCode, decimal value, decimal fictionalValue, DateTime? promotionStart, DateTime? promotionEnd,
            int? priceListId, int? skuId)
        {
            Id = id;
            Code = code;
        ...
        public static SkuPrice Criar(string code, string externalCode, decimal value, decimal fictionalValue, DateTime? promotionStart, DateTime? promotionEnd, 
            int? priceListId, int? skuId)
        {
            var advertisementSkuPrice = new SkuPrice();
            ...
            advertisementSkuPrice.SkuId = skuId;
        ...
        {
            ValidationResult = new SkuPriceModelValidation().Validate(this);
            return ValidationResult.IsValid;
        }

        public void MergeToUpdate(SkuPrice newSkuPrice)
        {
            this.Id = newSkuPrice.Id;
            this.Code = newSkuPrice.Code;
            this.ExternalCode = newSkuPrice.ExternalCode;
            this.Value = newSkuPrice.Value;
            this.FictionalValue = newSkuPrice.FictionalValue;
            this.PromotionStart = newSkuPrice.PromotionStart;
            this.PromotionEnd = newSkuPrice.PromotionEnd;
            this.PriceListId = newSkuPrice.PriceListId;
            this.SkuId = newSkuPrice.SkuId;
            this.UpdatedDate = newSkuPrice.UpdatedDate;
        }

        #endregion [ FIM - Metodos ]
    }

    public class SkuPriceModelValidation : AbstractValidator<SkuPrice>
    {
        public SkuPriceModelValidation()
        {
            ...
            RuleFor(x => x)
                .Cascade(CascadeMode.Stop)
                .NotEmpty()
                .IsSkuPriceCodeUnique().WithMessage(x => $"O Código {x.Code} já existe no banco de dados");
            ...
            {
                RuleFor(x => x.SkuId)
                    .SkuExists().WithMessage(x => $"O SkuId {x.SkuId} não faz referência a nenhum sku no banco de dados");
            });
        }
    }
}
